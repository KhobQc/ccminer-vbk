...
# MAIN script body
#######################
	threads=`echo "threads" | nc -w $API_TIMEOUT localhost 4058` #&& echo $threads
	if [[ $? -ne 0 || -z $threads ]]; then
		echo -e "${YELLOW}Failed to read $miner stats from localhost:4068${NOCOLOR}"
	else
		summary=`echo "summary" | nc -w 2 localhost 4058`
		...
		for (( i=0; i < ${#cckhs[@]}; i++ )); do
			...
			#cckhs[$i]="84316579.94" #test check Ghs. 1080ti gives ~64mh (64000kh) on lyra. when it's showing ghs then load is 0 on gpu if [[ `echo ${cckhs[$i]} | awk '{ print ($1 >= 1000000) ? 1 : 0 }'` == 1 ]]; then #hash is in Ghs, >= 1000000 khs
